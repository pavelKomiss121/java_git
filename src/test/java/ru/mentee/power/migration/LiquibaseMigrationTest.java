...
class LiquibaseMigrationTest extends BaseIntegrationTest {
    ...
    try (Connection conn = getTestConnection();
      ...
      ResultSet rs = stmt.executeQuery(
          "SELECT table_name FROM information_schema.tables " +
              "WHERE table_schema = 'mentee_power' AND table_type = 'BASE TABLE'"
      );
      ...
      try (ResultSet rs2 = stmt.executeQuery(
          "select to_regclass('public.databasechangelog') is not null as ok1, " +
              "       to_regclass('public.databasechangeloglock') is not null as ok2")) {
        rs2.next();
  ...
  void shouldTrackAllSqlChangesInChangelog() {
    try (Connection conn = getTestConnection(); Statement stmt = conn.createStatement()) {
      ResultSet rs = stmt.executeQuery(
          "select id, author from public.databasechangelog order by dateexecuted");
      List<String> ids = new ArrayList<>();
      List<String> authors = new ArrayList<>();
      while (rs.next()) { ids.add(rs.getString(1)); authors.add(rs.getString(2)); }

      assertThat(ids).contains(
          "001-create-schema", "002-create-users-table", "003-create-orders-table",
          "005-create-products-table", "004-add-user-phone", "004-fix-phone-regex",
          "dev-001-insert-test-users", "dev-002-insert-test-products");
  ...
  void shouldHandleReapplyingSqlMigrations() {
    try (Connection conn = getTestConnection()) {
      Database database = DatabaseFactory.getInstance()
          .findCorrectDatabaseImplementation(new JdbcConnection(conn));
      Liquibase lb = new Liquibase("db/changelog.yaml", new ClassLoaderResourceAccessor(), database);

      try (Statement s = conn.createStatement();
           ResultSet before = s.executeQuery("select count(*) from public.databasechangelog")) {
        before.next();
        int countBefore = before.getInt(1);
        lb.update("dev,test");
        try (ResultSet after = s.executeQuery("select count(*) from public.databasechangelog")) {
          after.next();
  ...
  void shouldValidateDataIntegrityAfterSqlMigrations() {
    try (Connection conn = getTestConnection(); Statement stmt = conn.createStatement()) {
      ...
      assertThatThrownBy(() -> stmt.executeUpdate(
          "INSERT INTO users(name,email) VALUES ('Dup','alex@menteepower.com')"))
          .isInstanceOf(Exception.class);

      // check phone
      assertThatThrownBy(() -> stmt.executeUpdate(
          "INSERT INTO users(name,email,phone) VALUES ('Bad','bad@x.com','123-456')"))
          .isInstanceOf(Exception.class);

      // fk orders.user_id must reference users.id (use non-existing id)
      assertThatThrownBy(() -> stmt.executeUpdate(
          "INSERT INTO orders(user_id,total_price,status) VALUES (999999,'10.00','PENDING')"))
          .isInstanceOf(Exception.class);
    } catch (Exception e) {
...
class LiquibaseSqlRollbackTest extends BaseIntegrationTest {
  ...
  protected void setUp() throws SASTException, IOException, DataAccessException {
    ...
    try (Connection conn = getTestConnection()) {
      Database database = DatabaseFactory.getInstance()
          .findCorrectDatabaseImplementation(new JdbcConnection(conn));
      liquibase = new Liquibase("db/changelog.yaml", new ClassLoaderResourceAccessor(), database);
      liquibase.update("dev,test");
    } catch (Exception e) {
  ...
  void shouldRollbackLastSqlMigrationCorrectly() throws Exception {
    try (Connection conn = getTestConnection(); Statement stmt = conn.createStatement()) {
      // before count
      ResultSet rs = stmt.executeQuery("select count(*) from mentee_power.databasechangelog");
      rs.next();
      int before = rs.getInt(1);

      liquibase.rollback(1, "dev,test");

      ResultSet rs2 = stmt.executeQuery("select count(*) from mentee_power.databasechangelog");
      rs2.next();
