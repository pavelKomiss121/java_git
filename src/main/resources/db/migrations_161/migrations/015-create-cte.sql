...
CREATE TABLE mentee_power.organizations (
...
CREATE TABLE mentee_power.employees (
                           id BIGSERIAL PRIMARY KEY,
...
-- Территории продаж
CREATE TABLE mentee_power.sales_territories (
                                   id BIGSERIAL PRIMARY KEY,
                                   name VARCHAR(100) NOT NULL,
                                   region VARCHAR(50),
                                   country VARCHAR(50),
...
ALTER TABLE mentee_power.customers ADD COLUMN IF NOT EXISTS territory_id BIGINT REFERENCES sales_territories(id);
...
ALTER TABLE mentee_power.orders ADD COLUMN IF NOT EXISTS sales_rep_id BIGINT REFERENCES employees(id);
